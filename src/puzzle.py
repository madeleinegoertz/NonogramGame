...
class Puzzle(object):
    ...
    def __init__(self, name: str, row_clues_path: str, col_clues_path: str):
        self._row_clues = [[int(n) for n in line.split()] for line in open(row_clues_path).read()]
        self._col_clues = [[int(n) for n in line.split()] for line in open(col_clues_path).read()]
    ...
    def _max_digits(clues) -> int:  # type = clues: Line[int][int]
        ...
        for i in range(len(clues)):
            for j in range(len(clues[i])):
                num_digits = math.ceil(math.log10(clues[i][j]))
